...
//**
	...
	constructor() ERC721("GameVerse", "GMV") {
        ...
        _REWDTYPE[0]=NFTST("0.json",100,1,360,false,true,address(0));
        _REWDTYPE[1]=NFTST("0.json",250,1,180,false,true,address(0));
        _REWDTYPE[2]=NFTST("0.json",500,2,360,false,true,address(0));
        _REWDTYPE[3]=NFTST("0.json",250,2,180,false,true,address(0));
        _NFTTYPE[0]=NFTST("0.json",500,1,360,false,false,address(0));
        _NFTTYPE[1]=NFTST("0.json",250,1,180,false,false,address(0));
        _NFTTYPE[2]=NFTST("0.json",500,2,360,false,false,address(0));
        _NFTTYPE[3]=NFTST("0.json",250,2,180,false,false,address(0));
    }
   ...
   /* function rewdtypeRD(uint256 tid) public view onlyOwner returns (uint256 cnt,NFTST memory rst)
    ...
    function referalsend(address radd, uint256 rtype, uint256 num, bool lck) public onlyOwner {
        airdropmint(radd, rtype, num, lck);
    }
    function airdropmint(address radd, uint256 rtype, uint256 num, bool lck) private {
    ...
    function bulkmint(uint256 blknum,uint256 nfttype, address radd,uint256 tkid)
        ...
        if(radd!=address(0)){
            airdropmint(radd, nfttype,blknum,false);
        } else {
	...
	function CustomMint(string memory _uri, address radd, uint256 nfttype, uint256 tkid) public payable returns (uint256) {
		...
		_setTokenURI(newItemId, _uri);
        if(radd!=address(0)){
            airdropmint(radd, nfttype,1,false);
        } else {
